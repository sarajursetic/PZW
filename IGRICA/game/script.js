...
// Ažurirano: Slika za krajnji portal (Added: Image for the end portal)
const endPortalImg = new Image();
endPortalImg.src = 'images/scenery/portal.png'; // Pretpostavka da je ovo sada sprite sheet
...
class Player {
    constructor() {
        ...
        this.facingRight = true; // Direction the player is facing
    }
...
class Ground {
    constructor() {
        this.height = 100;
        this.tileWidth = 100; // Width of a single ground tile
        this.width = canvas.width * 2; // Wider than screen for scrolling
...
class Monster {
    ...
    update() {
        ...
        if (this.checkCollision()) {
            ...
            if (playerBottom <= monsterTop + 20 && player.velocity.y > 0) {
                ...
                this.isDead = true;
                player.velocity.y = -15; // Bounce player up
    ...
    reset() {
        this.isDead = false;
        this.position.x = 3500; // Place after platforms
        this.position.y = ground.getGroundY() - this.height;
        this.facingRight = true;
    }
}
...
const monster = new Monster(3500, 0); // Place monster after platforms
const platforms = [
...
const endPortal = new EndPortal(3800, ground.getGroundY() - 64);
...
function animate() {
    ...
    if(keys.right.pressed && player.position.x < 400){
        ...
        if(hasGroundAhead || onPlatform) {
            ...
            player.velocity.x = 0; // Prevent movement if no ground/platform
        }
    } else if(keys.left.pressed && player.position.x > 100 && scrollOffset === 0){
        // Check if there's ground behind the player
        const nextX = player.position.x - 5;
        ...
        if(keys.right.pressed){
            ...
            monster.position.x -= 5; // Add monster scrolling
            player.facingRight = true;
